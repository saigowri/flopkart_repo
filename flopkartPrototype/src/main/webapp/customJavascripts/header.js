...
function categoryMenu(result) {
	for ( var i in result) {
		...
		var data = " <a href='category.jsp'  style='color:black' data-hover='dropdown' "
				...
				+ "<span id='span_"
				+ result[i].id
				+ "' style='color:grey' class='glyphicon glyphicon-chevron-down'></span>"
				+ "</a>";
...
function findUserByPhone(ctxPath) {
	$.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : ctxPath + "/webapi/users/phone",
		dataType : "json", // data type of response
		data : formToJSONPhone(),
		success : renderDetails
	});
}
